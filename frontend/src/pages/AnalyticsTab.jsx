...
const AnalyticsTab = () => {
  ...
  const handleFetchData = async () => {
    setIsLoading(true);
    try {
      const res = await axios.post('http://localhost:8000/api/analytics', {
  ...
  const handleExportFile = async (formatType) => {
    try {
      const response = await axios.post(`http://localhost:8000/api/analytics/export/${formatType}`, {
        start_date: startDate,
        end_date: endDate,
        camera: selectedCam
            ...
            <div className="bg-[#11151A] border border-gray-700 rounded-xl p-5 shadow-lg">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-2/3 bg-black border border-gray-700 rounded-lg flex items-center justify-center overflow-hidden min-h-[300px]">
                  ...
                  <img 
                    src={`http://localhost:8000/evidence/violation_${data.evidence_list[selectedEvidenceIdx]['ID Xe']}.jpg`} 
